...
import {updatePermissionAPI} from "../../services/permission.service.ts";
...
const UpdatePermission = (props: Props) => {
  const {isOpen, onClose, getPermissionList, dataSelected} = props;
  const initialValues: FormValues = {
    id: dataSelected.id ?? null,
    name: dataSelected.name ?? '',
    apiPath: dataSelected.apiPath ?? '',
    method: dataSelected.method ?? '',
    module: dataSelected.module ?? ''
  };
  const toast = useToast();
  const methodList = ['POST', 'PATCH', 'GET', 'DELETE'];

  const handleSubmit = async (values: FormValues, actions: FormikHelpers<FormValues>) => {
    const res: ResponseType = await updatePermissionAPI(values);
  ...
  return (
    ...
    >
      ...
      <DrawerContent>
        <DrawerCloseButton/>
        <DrawerHeader>Update permission</DrawerHeader>
        <DrawerBody>
          <Formik
