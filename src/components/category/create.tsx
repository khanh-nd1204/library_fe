...
import {createCategoryAPI} from "../../services/category.service.ts";
...
const CreateCategory = (props: Props) => {
  const {isOpen, onClose, getCategoryList} = props;
  const initialValues: FormValues = {name: '', description: ''};
  const toast = useToast();

  const handleSubmit = async (values: FormValues, actions: FormikHelpers<FormValues>) => {
    const res: ResponseType = await createCategoryAPI(values);
