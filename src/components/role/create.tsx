...
const CreateRole = (props: Props) => {
  const {isOpen, onClose, getRoleList, permissionList} = props;
  const initialValues: FormValues = {name: '', description: '', permissions: []};
  const toast = useToast();

  const handleSubmit = async (values: FormValues, actions: FormikHelpers<FormValues>) => {
    const res: ResponseType = await createRoleAPI(values);
