...
import {createAuthorAPI} from "../../services/author.service.ts";
...
const CreateAuthor = (props: Props) => {
  const {isOpen, onClose, getAuthorList} = props;
  const initialValues: FormValues = {name: '', nationality: '', penName: ''};
  const toast = useToast();

  const handleSubmit = async (values: FormValues, actions: FormikHelpers<FormValues>) => {
    const res: ResponseType = await createAuthorAPI(values);
  ...
  return (
    ...
    >
      ...
      <DrawerContent>
        <DrawerCloseButton/>
        <DrawerHeader>Create author</DrawerHeader>
        <DrawerBody>
          <Formik
